    ...
    >
      ...
      >
        <motion.h2 className="text-4xl md:text-6xl xl:text-8xl text-white">
          {getJsonValue("Let's work together")}
        </motion.h2>
        {contactInfo.map(item => (
          <motion.div key={item.title} variants={variants}>
            <h3 className="text-white font-normal text-xl md:text-2xl">
              {getJsonValue(item.title)}
            </h3>
            <span className="font-light text-gray-300">{getJsonValue(item.info)}</span>
          </motion.div>
        ))}
      </motion.article>
      <article className="flex-1 relative w-full p-10 md:p-0">
        ...
        >
          ...
          <input
            type="email"
            name="email"
            placeholder={getJsonValue("Email")}
            required
            className="p-2 md:p-4 bg-transparent border border-white text-white rounded-md outline-none"
          />
          <textarea
            name="message"
            placeholder={getJsonValue("Message")}
            rows={4}
            required
            className="p-2 md:p-4 bg-transparent border border-white text-white rounded-md outline-none"
          />
          ...
          {isSuccess && <p className="text-green-500 text-xs">{getJsonValue("Email sent")}</p>}
        </motion.form>
      </article>
    </motion.div>
  );
